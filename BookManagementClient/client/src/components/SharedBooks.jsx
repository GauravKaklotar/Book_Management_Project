...
export default function SharedBooks() {
  ...
  const [centredModalForTake, setCentredModalForTake] = useState(false);

  const toggleShowForTake = () => setCentredModalForTake(!centredModalForTake);
  ...
  const handleTakeBake = async () => {
    try {
        ...
        const res = await fetch(`https://localhost:7285/api/BooksController1/${formValue.bookID}`, {
          ...
          body: JSON.stringify({
            bookID: formValue.bookID, bookTitle: formValue.bookTitle, bookAuthor: formValue.bookAuthor, sharedWith: '', isAvailable: true, isShared: false
          })
        });

        toast.success("Book has been taken successfully.");
        toggleShowForTake();
        // setTimeout(()=>window.location.reload(), 5000);
        window.location.reload();
      }
    ...
    <>
      <Box sx={{ mt: 5 + 'vh', ml: 5 + 'vh', mr: 5 + 'vh' }}>
        ...
        <MDBTable align='middle' responsive hover>
          ...
          <MDBTableBody>
            {query.length>0 ? filterData.filter(item => item.isShared).map((item, index) => (
              <tr key={index}>
                ...
                <td>
                  <p className='fw-normal mb-1'>{item.sharedWith}</p>
                </td>
                <td>
                  <MDBBtn color='info' rounded size='sm' className='me-2 mt-1' placement='bottom' title='Take BOOK' onClick={() => {
                    setFormValue({
                      bookID: item.bookID,
            ...
            )) : data.filter(item => item.isShared).map((item, index) => (
              <tr key={index}>
                <th scope='row'>{index + 1}</th>
                <td>
                  <div className='d-flex align-items-center'>
      ...
      <MDBModal tabIndex='-1' show={centredModal} setShow={setCentredModal}>
       ...
       {/* Take Back */}
       <MDBModal tabIndex='-1' show={centredModalForTake} setShow={setCentredModalForTake}>
        <MDBModalDialog centered>
          <MDBModalContent>
            <MDBModalHeader>
              <MDBModalTitle>TAKE BOOK</MDBModalTitle>
              <MDBBtn className='btn-close' color='none' onClick={toggleShowForTake}></MDBBtn>
            </MDBModalHeader>
            <MDBModalBody>
              <p>
                Are you sure you want to take back this book from <b>{formValue.sharedWith}</b>?
              </p>
            </MDBModalBody>
            <MDBModalFooter>
              <MDBBtn color='secondary' onClick={toggleShowForTake}>
                Close
              </MDBBtn>
              <MDBBtn color='warning' onClick={() => handleTakeBake()}>TAKE BOOK</MDBBtn>
            </MDBModalFooter>
          </MDBModalContent>
        </MDBModalDialog>
      </MDBModal>
